...
MLSA per state from 1992 to 2025

```{r}
ggplot( simulated_data) + 
geom_line(aes( x = Year, y = MLSA, color = state ) ) + 
theme_bw( base_size = 12) + 
theme( legend.position = "none" ) + 
expand_limits( y = 0) +
labs( x = "Year", y = "MLSA", 
title = "MLSA per state from 1992 to 2025")
```

top ten states with lowest MLSA from 1992 to 2025.

```{r}
# View( simulated_data |> arrange( desc( MLSA )))

top10states <- simulated_data |> 
  arrange(desc(MLSA)) |>
  slice_head(n = 10 ) |>
  pull( state )

ggplot( simulated_data |>
      filter( state %in% top10states ) ) +
geom_line(aes( x = Year, y = MLSA, color = state) ) + 
theme_bw( base_size = 12) + 
theme( legend.position = "bottom" ) + 
expand_limits( y = 0) +
labs( x = "Year", y = "MLSA", 
title = "10 lowest MLSAs from 1992 to 2025")
```
